...
- 当前 Work Package：M0.5-WP03 — GitHub Actions CI（BLOCKED：等待 PR 创建）。
...
M0.5-WP03 GitHub Actions CI：CI workflow 已完成本地验证并推送到专用分支；当前因本机缺少 GitHub CLI 而阻塞在 PR 创建，远程 CI 尚未触发。禁止直接 push main、force push、自动 merge、共享分支 rebase、远程历史重写、生产服务/数据库/n8n 配置变更；在 PR 建立前不得开始 WP04。
